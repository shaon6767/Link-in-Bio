...
export const getLinks = async (req: AuthRequest, res: Response) => {
  try {
    const links = await Link.find({ userId: req.user!.userId }).sort({ order: 1, createdAt: 1 });
    res.json(links);
  } catch (error) {
...
export const createLink = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const lastLink = await Link.findOne({ userId: req.user!.userId }).sort({ order: -1 });
...
export const updateLink = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.status(500).json({ message: 'Error updating link', error: (error as Error).message });
  }
};
...
export const deleteLink = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const link = await Link.findOneAndDelete({ _id: id, userId: req.user!.userId });
    if (!link) {
      return res.status(404).json({ message: 'Link not found' });
    }
...
export const reorderLinks = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updatedLinks = await Link.find({ userId: req.user!.userId }).sort({ order: 1 });
    res.json(updatedLinks);
  } catch (error) {
    res.status(500).json({ message: 'Error reordering links', error: (error as Error).message });
  }
};
